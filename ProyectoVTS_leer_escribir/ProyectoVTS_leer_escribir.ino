...
//Wifi
char ssid[] = "Fernando's Galaxy A32";      // your network SSID (name)
char pass[] = "gxho1218";   // your network password
...
void loop() {
  ...
  ThingSpeak.setField(2, caudal_L_m[1]);
  //ThingSpeak.setField(3, caudal_L_h[0]);
  //ThingSpeak.setField(4, caudal_L_h[1]);
  ThingSpeak.setField(3, kPa[0]);
  ThingSpeak.setField(4, kPa[1]);
  ThingSpeak.setField(5, kPa[2]);
  ThingSpeak.setField(6, kPa[3]);
  ThingSpeak.setField(7, kPa[4]);
...
void actuacion(void){
  ...
  if(statusCode == 200){
    ...
    if(Resultado==1)Serial.println("*******************Lectura = 1++++++++++++++" );
  }
  else{
